<template>
    <div>
        <div class="form-group">
            <label>ФИО</label>
            <input v-model="model2.full_name" type="text" class="form-control" name="full_name">
        </div>

        <div class="form-group">
            <label>Дата рождения</label>
            <input v-model="model2.birthday_date" type="date" class="form-control" name="birthday_date">
        </div>

        <div class="form-group">
            <label>Организация</label>
            <input v-model="model2.organization" type="text" class="form-control" name="organization" disabled>
        </div>

        <div class="form-group">
            <label>Тип пользователя</label>
<!--            <select class="form-control" v-model="model2.user_type_id" name="user_type_id">-->
            <select class="form-control" v-model="f" name="user_type_id">
                <option :value="key" v-for="(value, key) in UserTypeArr">
                    {{value}}
                </option>
            </select>
        </div>

        <div class="form-group">
            <label>Блок с чекбоксами</label>
            <ul>
<!--                <li v-for="(value, key) in checkboxArr[model2.user_type_id]">-->
                <li v-for="(value, key) in checkboxArr[f]">
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" :id="'check'+key" v-model="value.checked"
                               :name="'role['+value.id+']'">
                        <label class="form-check-label" :for="'check'+key">{{value.title}}</label>
                    </div>
                </li>
            </ul>
        </div>
        <input type="hidden" :value="model2.id" name="id">


<!--        <button type="button" class="btn btn-light" @click="window.location.href='/index'">Назад</button>-->
        <a href="/" class="btn btn-light">Назад</a>
        <button type="submit" class="btn btn-primary">Сохранить</button>
    </div>
</template>

<script>
    export default {
        props: [
            'model',
            'checkbox_arr',
            'user_type_arr',
        ],
        mounted() {
            // console.log(1);
            console.log(Object.keys(this.user_type_arr)[0]);
        },
        data: function () {
            return {
                f: this.model.user_type_id,
                model2: this.model,
                checkboxArr: this.checkbox_arr,
                UserTypeArr: this.user_type_arr,
            }
        }
    };
</script>
